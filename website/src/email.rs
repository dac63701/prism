                ...
                <tr>
                  <td style="border-top:1px solid #1f2a44;padding-top:20px;">
                    <p style="margin:0;font-size:13px;line-height:1.5;color:#52525b;">
                      If you did not create an account on Prism, you can safely ignore this email.
                    </p>
                  </td>
                </tr>
              </table>
...
pub async fn send_verification_email(
    ...
    for attempt in 1..=2 {
        let email = build_verification_email(config, to_email, to_name, token)?;
...
fn two_factor_email_html(username: &str, code: &str, site_url: &str) -> String {
    let logo_url = format!("{}/brand/logo.svg", site_url.trim_end_matches('/'));
    format!(r##"<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Your Prism 2FA Code</title>
</head>
<body style="margin:0;padding:0;background-color:#050816;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Oxygen,Ubuntu,sans-serif;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#050816;">
    <tr>
          ...
          <tr>
            <td style="background-color:#0b1222;border:1px solid #1f2a44;border-radius:24px;padding:40px 32px;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td align="center" style="padding-bottom:8px;">
                    <span style="font-size:11px;text-transform:uppercase;letter-spacing:0.3em;color:#77a8ff;">Security Verification</span>
                  </td>
                </tr>
                <tr>
                  <td align="center" style="padding-bottom:16px;">
                ...
                <tr>
                  <td align="center" style="padding-bottom:12px;">
                    <p style="margin:0;font-size:15px;line-height:1.6;color:#a1a1aa;">
                      Hi <strong style="color:#e5eefc;">{username}</strong>,
                    </p>
                  </td>
                </tr>
                <tr>
                    ...
                    <table role="presentation" cellpadding="0" cellspacing="0" style="margin:0 auto;">
                      <tr>
                        <td style="background:#10192e;border:1px solid #1f2a44;border-radius:16px;padding:20px 32px;">
                          <p style="margin:0;font-size:36px;font-weight:700;letter-spacing:0.15em;color:#e5eefc;font-family:monospace;text-align:center;">{code}</p>
                        </td>
                      </tr>
                    </table>
                  </td>
                ...
                <tr>
                  <td style="border-top:1px solid #1f2a44;padding-top:20px;">
                    <p style="margin:0;font-size:13px;line-height:1.5;color:#52525b;">
                      If you did not attempt to sign in to Prism, you can safely ignore this email.
                    </p>
                  </td>
                </tr>
              </table>
...
fn build_2fa_email(config: &Config, to_email: &str, to_name: &str, code: &str) -> Result<Message, String> {
    let html = two_factor_email_html(to_name, code, &config.site_url);
    ...
    Message::builder()
        ...
        .to(format!("{to_name} <{to_email}>")
            .parse()
            .map_err(|e| format!("Invalid to address: {e}"))?)
        .subject("Your Prism Two-Factor Authentication Code")
        .header(ContentType::TEXT_HTML)
        .body(html)
        .map_err(|e| format!("Failed to build 2FA email: {e}"))
}

pub async fn send_2fa_email(config: &Config, to_email: &str, to_name: &str, code: &str) -> Result<(), String> {
    if config.smtp_host.is_empty() {
        tracing::error!(to = %to_email, "SMTP not configured — 2FA email not sent");
        return Err("SMTP is not configured. Set SMTP_HOST, SMTP_USERNAME, and SMTP_PASSWORD.".into());
    }
    ...
    for attempt in 1..=2 {
        let email = build_2fa_email(config, to_email, to_name, code)?;
        match mailer.send(email).await {
            Ok(_) => {
                tracing::info!(to = %to_email, attempt, "2fa email sent");
                return Ok(());
            }
            Err(e) => {
                if attempt == 2 {
                    return Err(format!("Failed to send 2FA email after two attempts: {e}"));
                }
                tracing::warn!(to = %to_email, attempt, error = %e, "2FA email delivery failed; retrying");
                tokio::time::sleep(Duration::from_secs(1)).await;
            }
        }
    }

    unreachable!("the 2FA email retry loop always returns")
}

#[cfg(test)]
mod tests {
    use super::{verification_email_html, two_factor_email_html};
    ...
    #[test]
    fn two_factor_email_includes_code() {
        let html = two_factor_email_html(
            "Test User",
            "987654",
            "https://goprism.studio/",
        );
        ...
        assert!(html.contains("Your Two-Factor Authentication Code"));
